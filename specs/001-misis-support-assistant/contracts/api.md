...
Единый контракт для веб-фронта (002) и Telegram-бота (003). Формат: JSON, UTF-8. Аутентификация: cookie `session_id` (HttpOnly), кроме помеченных «публичный». Веб ходит через Vite-прокси `/api` → `:8000`; бот — напрямую на `:8000`. Ошибки: `{ "detail": "текст по-русски" }` со статусами 400/401/403/404/409/422/500.
...
### GET /api/auth/me
```json
← 200 { "user": {…} } | ← 401 { "detail": "Требуется вход" }
```

## Обращения (пользователь)

### POST /api/requests
Подача обращения. Запускает конвейер агента синхронно и возвращает результат первой реакции.
```json
...
← 200 {
  ...
  "reactions": [
    { "kind": "clarification",         // "answer" | "clarification" | "outage_notice" | "cert_ordered" | "escalated"
...
← 401 | 422 (пустой текст)
```

### POST /api/requests/{id}/reply
Ответ пользователя на уточняющий вопрос (диалог, ≤ 2 раундов — FR-025).
```json
...
### GET /api/requests — мои обращения (пользователь из сессии).
```json
...
### GET /api/requests/{id} — детально: обращение + подзадачи + диалог + журнал.
```json
