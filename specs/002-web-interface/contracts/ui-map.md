...
- 401 на любом запросе → очистка session, переход на `login`.
...
|---|---|---|
| `login` | `POST /api/auth/login` | вход; показ ошибки домена |
| `request` | `POST /api/requests`, `POST /api/requests/{id}/reply`, `GET /api/requests/{id}` | примеры-кнопки; результат с badges; диалог; плашка сбоя |
| `cabinet` | `GET /api/requests`, `GET /api/certs/catalog`, `POST /api/certs/orders`, `GET /api/certs/orders` | заказ из каталога; цепочка статусов; polling 5 c |
...
- `reaction.kind === "outage_notice"` → красная плашка; `clarification` → сообщение агента + активный ввод; `answer` → сообщение агента; `cert_ordered` → карточка «Заказ №N создан» + ссылка в кабинет; `escalated` → плашка «Передано специалисту».
